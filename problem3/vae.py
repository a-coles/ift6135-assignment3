...
class Encoder(nn.Module):
    def __init__(self, device='cpu'):
        ...
        self.fc_mu = nn.Linear(in_features=1024, out_features=100)
        self.fc_logvar = nn.Linear(in_features=1024, out_features=100)
        self.elu = nn.ELU()
        self.avg_pool = nn.AvgPool2d(kernel_size=2, stride=2)

    def forward(self, inp):
        # inp = inp.float().view(-1, 32, 32).unsqueeze(1)
        # print('input after view is', inp.size())
        out = self.conv1(inp)
...
class VAEArch(nn.Module):
    ...
    def forward(self, inp):
        ...
        # out = out.squeeze(1).view(-1, 32 * 32)
        # Re-flatten for loss calc
